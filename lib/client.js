...
/**
 * uploadCSV function - will wrap callback in error handler
 *
 * @param path {String} - base path
 * @param mimeType string - mime type
 * @param fileContent Buffer/string - file content
 * @param callback {Function}
 */
ZuoraClient.prototype.upload = function (path, filename, contentType, fileContents, callback) {
    path = restAddon + path;
    var log = this.log.child({method: 'POST', path: path});
    this.client.upload(path, filename, contentType, fileContents, error.getHandler(log, callback));
};
